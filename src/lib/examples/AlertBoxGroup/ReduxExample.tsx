...
const ReduxExample = () => (
  <div>
    <h3>Redux example</h3>
      <p>With this example we use <code>Redux</code> store manager where we store Alert list. Just create few alerts using options provided and observe the component's behaviour.</p>
      <p>The reducer consumes <code>addAlert</code> helper to correctly update the list. It's happening using <code>actionAddAlert</code> action object exposed to lower components. If lower components will need it (i.e. because of Api call for data save ended up with success) they just call new alert's addition.</p>
      <p>The reducer also consumes <code>dismissAlert</code> helper to correctly remove given alert from the list. As alert will disappear (by self collapse, or by clicking close button) there is no need to store it on a memory anymore. This is the reason for mentioned dismissal.</p>
      <GlobalStore>
          <GlobalViewWrapper />
      </GlobalStore>
  </div>
